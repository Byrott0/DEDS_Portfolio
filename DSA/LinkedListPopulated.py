...
def largest(self):
    if len(self) == 1:
        return self[0]
    return max(self[0], largest(self[1:]))
print(largest([5, 4, 3, 2, 7]))  # Output: 7

def sortSimple(lst):
    if not lst:
        return []
    smallest_value = smallest(lst)
    largest_value  = largest(lst)
    return [largest_value] + sortSimple(remove(largest_value, lst))
